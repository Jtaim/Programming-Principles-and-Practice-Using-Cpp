...
bool Roman_int::romanToDecimal( std::string &symbols )
{
    roman_int = 0;
    ...
    for( auto symbol_itr{ symbols.cbegin() }; symbol_itr != symbols.cend(); ++symbol_itr )
    {
        ...
        auto symbol_value{ find( look_up, static_cast<char>( std::toupper( static_cast<unsigned char>( *symbol_itr ) ) ) ) };
        ...
        {
            auto next_symbol_value{ find( look_up, static_cast<char>( std::toupper( static_cast<unsigned char>( *next_symbol_itr ) ) ) ) };
