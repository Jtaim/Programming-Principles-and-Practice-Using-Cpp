...
int main()
{
    try
    {
        ...
        while( std::cin >> p_c )
        {
            p_c = narrow_cast<char>( tolower( static_cast<unsigned char>( p_c ) ) );
            if( p_c != 'p' && p_c != 'c' )
            {
        ...
        std::cout << std::format( "The {} is {}.\n",
                                  ( p_c == 'p' ? "permutation" : "combination" ),
                                  ( p_c == 'p' ? permutation( a, b ) : combination( a, b ) ) );
    }
