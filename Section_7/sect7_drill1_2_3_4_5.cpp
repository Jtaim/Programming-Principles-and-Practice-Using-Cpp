...
// place to hold variable name, value pairs
struct Variable
{
    std::string name;
    double value{};
...
{
    auto vt_itr = std::find( variables.cbegin(), variables.cend(), s );
    if( vt_itr == variables.cend() )
    {
...
{
    auto vt_itr = std::find( variables.begin(), variables.end(), s );
    if( vt_itr == variables.cend() )
    {
...
{
    return variables.cend() != std::find( variables.cbegin(), variables.cend(), s );
}
...
double define_name( const std::string s, const double d )
{
    ...
    variables.push_back( Variable{ s, d } );
    return d;
}
